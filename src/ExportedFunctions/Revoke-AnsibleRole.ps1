function Revoke-AnsibleRole {
    ...
    process {
        switch ($Team.GetType().Fullname) {
            ...
            "System.String" {
                ...
                if($Team.Count -gt 1) {
                    ...
                    if(!$Force -and !$PSCmdlet.ShouldContinue("Revoke role from multiple teams? (Use -Force to bypass prompt)", "Multiple teams found: $TeamList")) {
                        return
                    }
                }
            }
        ...
        $Team | ForEach-Object {
            ...
            $Role | ForEach-Object {
                $ThisRole = $_
                if($PSCmdlet.ShouldProcess("$($ThisRole.Resource_type)/$($ThisRole.resource_name)", "Revoke $($ThisRole.Name) from $($ThisTeam.Organization)/$($ThisTeam.Name)")) {
                    Invoke-PostAnsibleInternalJsonResult -ItemType teams -ItemId $ThisTeam.Id -ItemSubItem roles -InputObject @{ id = $ThisRole.Id; disassociate = $true } -AnsibleTower $AnsibleTower
                }
            }
        }
    }
